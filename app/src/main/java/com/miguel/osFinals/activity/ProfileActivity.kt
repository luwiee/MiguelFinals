...
class ProfileActivity : AppCompatActivity() {
    ...
    private fun uploadImage() {
        if (filePath != null) {
            ...
            var ref: StorageReference = storageRef.child("image/" + etUserName.text.toString() +"/"+ UUID.randomUUID().toString())
            ref.putFile(filePath!!)
                .addOnSuccessListener {
                    Glide.with(this@ProfileActivity).load(filePath).into(userImage)
                    ref.downloadUrl.addOnSuccessListener{
                        val hashMap:HashMap<String,String> = HashMap()
                        hashMap.put("userName",etUserName.text.toString())
                        hashMap.put("profileImage", it.toString())
